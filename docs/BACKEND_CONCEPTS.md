...
### Why Alembic Sometimes Ignores a Model
Alembic generates migrations by inspecting `Base.metadata`.

A model is included in `Base.metadata` only after Python imports the module containing that model.

__Therefore__:
- Defining a model file is not enough.
- The model module must also be imported before Alembic performs autogeneration.

>If a model is missing from the migration:
1. Verify it inherits from the project's shared `Base`.
2. Verify the module is imported during Alembic initialization.
3. Verify the table appears in `Base.metadata.tables`.
...
- Alembic tracks migrations in two places:

1. Migration files stored in the project's `alembic/versions/` directory.
2. The `alembic_version` table inside the database.

- Before generating a new migration, Alembic verifies that the database has already applied all existing migration files.

- If previous migrations have not been applied, Alembic raises:
Target database is not up to date.

- This prevents generating new migrations on top of an outdated database schema.

### Recommended Workflow
1. Generate migration.
2. Review migration.
3. Apply migration.
4. Only then generate the next migration.

- This keeps the migration history linear and consistent.
...
### Relationship Cascade vs Foreign Key Cascade
There are two different kinds of cascade in SQLAlchemy.

>Database Cascade
Defined inside a foreign key.
...
When the parent row is deleted, the database automatically deletes dependent rows.

>ORM Cascade
Defined inside `relationship()`.
...
This is enforced by SQLAlchemy before SQL statements are sent to the database.

>Difference
Database Cascade
- Works inside the database.
- Independent of SQLAlchemy.

ORM Cascade
- Works inside SQLAlchemy.
- Controls how ORM objects are persisted and deleted.

Both can be used together, but they solve different problems.
...
### TYPE_CHECKING in Python
`TYPE_CHECKING` is a special constant provided by Python's `typing` module.

```python
from typing import TYPE_CHECKING
```

It is used to write imports that are required only for static type checking and editor support.
...
The import is skipped completely.
...
This allows the editor to understand:
- class definitions
- type hints
- autocomplete
- error detection
...
### Why It Is Useful
Large projects often have models that reference each other.
...
Direct imports can create circular import problems.

Using `TYPE_CHECKING` keeps type hints available while preventing runtime import cycles.
...
This is the recommended pattern for SQLAlchemy ORM models in professional Python projects.

### Benefits
- Prevents circular imports.
- Keeps editor autocomplete.
- Preserves static type checking.
- Makes large codebases easier to maintain.

### Verifying SQLAlchemy Metadata
Before generating migrations, verify that all models are registered with SQLAlchemy.
...
If a table is missing from `Base.metadata.tables`, Alembic cannot generate a migration for it.

Always verify metadata before investigating Alembic itself.
...
In SQLAlchemy ORM models:
- `Mapped[...]` should use Python types.
- `mapped_column(...)` should use SQLAlchemy column types.
...
__Correct mapping__:
| Python Type (`Mapped`) | SQLAlchemy Column |
|-------------------------|-------------------|
| `int` | `Integer` |
| `str` | `String` |
| `bool` | `Boolean` |
...
The Python type represents the value used by the application.

The SQLAlchemy type represents how the value is stored in the database.
...
## Passlib & bcrypt
>Definition
A password hashing library for Python that provides over 30 password hashing algorithms. bcrypt is the industry-standard algorithm we use with it.

>Purpose
To securely convert plain-text passwords into irreversible hashes before storing them in the database, protecting user data if the database is ever compromised.

>Industry Practice
Never store plain-text passwords. Always salt and hash them. Note: In newer versions of bcrypt (5.0.0+), passlib may throw errors due to removed compatibility wrappers, requiring bcrypt to be pinned to `<4.0.0` or `4.0.1`.
...
>Purpose
Used for stateless authentication. Instead of storing session IDs in the database, the server generates a cryptographically signed token (JWT) containing the user ID. The client sends this token with every request to prove their identity.

>HelloStay Usage
Generated by `python-jose` when an Owner or Employee logs in. The token is sent back to the React frontend to be attached to future API calls.

---

## CORS Middleware Configuration
>Definition
Cross-Origin Resource Sharing (CORS) middleware is configured on the FastAPI server to explicitly allow requests from the frontend origin during development and production.

>Purpose
Browsers block JavaScript from making HTTP requests to a different origin (domain, port, protocol) unless the server explicitly permits it via CORS headers. The middleware adds these headers to every response.
...
>Industry Practice
- Restrict `allow_origins` to known frontend URLs in production — never use `["*"]`
- `allow_credentials=True` is required when using cookies or Authorization headers
- `allow_methods=["*"]` is safe — the browser still respects the actual HTTP method
- The middleware runs on every response, so it's server-wide, not route-specific
- In production, consider using a reverse proxy (Nginx) instead of CORSMiddleware for stricter control

>Common Mistakes
- Using `allow_origins=["*"]` with `allow_credentials=True` — browser rejects the request (spec violation)
- Forgetting the port number in localhost — `http://localhost:5173` differs from `http://localhost`
- Relying solely on Vite proxy during development without CORS configured for production

>HelloStay Usage
Configured in `backend/app/main.py` lines 12-18. Allows only `http://localhost:5173` (the Vite dev server). Since the frontend also has a Vite proxy for development, CORS acts as a fallback for direct API calls.

---

## Router Registration Pattern
>Definition
API routes are organized into separate router modules under `app/api/`, each defining a tagged router with `APIRouter(prefix=...)`, then registered in the main app via `app.include_router()`.

>Purpose
Keeps route definitions modular — each domain (rooms, guests, stays) has its own file. Routes are grouped under a common prefix and Swagger tag for clear API documentation.
...
>Industry Practice
- One router module per domain entity (room, guest, stay)
- Use `prefix` to avoid repeating the path in every route decorator
- Use `tags` for Swagger/OpenAPI grouping
- Import with `as` alias for clarity: `room_router`, `guest_router`
- Register all routers centrally in `main.py`

>Common Mistakes
- Putting all routes in `main.py` — file becomes unmanageable
- Forgetting the prefix — routes register at root level and conflict
- Not tagging routes — Swagger UI shows all endpoints under "Default"

>HelloStay Usage
Five routers registered in `backend/app/main.py` lines 20-24: `system_info`, `room`, `guest`, `stay`, `guest_stay`. Each lives in `backend/app/api/<name>.py`.

---

## Backend Application Structure
>Definition
The backend follows a layered architecture with separate directories for models (SQLAlchemy tables), schemas (Pydantic validation), API (route handlers), and database (connection/session management).
...
>Purpose
Separation of concerns: models define the database schema, schemas define API contracts, API modules handle business logic, and database handles connections. This makes the code testable, maintainable, and scalable.

>Industry Practice
- Models should never import from API modules (avoids circular imports)
- Schemas are independent from models — allows API response shape to differ from DB schema
- Database session is created per-request via FastAPI dependency injection
- Alembic handles schema migrations — never manually alter production tables

>Common Mistakes
- Mixing Pydantic schemas with SQLAlchemy models — leads to tight coupling and migration issues
- Importing models inside route handlers creates circular imports when models reference schemas
- Not using Alembic — schema drift between local and production databases

>HelloStay Usage
The `backend/` project follows this structure exactly. Each API router imports its corresponding model and schema from the `models/` and `schemas/` packages.
